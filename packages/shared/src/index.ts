...
export interface UserProfile {
  ...
  pinHash?: string; // Optional securely hashed passcode/PIN for the profile
  hasSkippedPin?: boolean; // Track if the user chose to skip setting a PIN
}
...
export interface Transaction {
  ...
  isDuplicate?: boolean;
  isAutoGenerated?: boolean; // Set by automation engine (recurring txns, dividends) for idempotency checks
}
...
export const filterByDateRange = <T>(
  ...
  const rawStart = dateRange.startDate ? new Date(dateRange.startDate).getTime() : 0;
  const rawEnd = dateRange.endDate ? new Date(dateRange.endDate + 'T23:59:59.999Z').getTime() : Infinity;
